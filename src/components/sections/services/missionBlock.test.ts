...
/**
 * Component-layer test for the Services-page mission block (ADR-0051).
 *
 ...
 *    display strip, restoring the specialisation labels ADR-0051
...
describe('MissionBlock (component layer, real coach data)', () => {
  ...
  it('emits no `credentialLine` value on this surface', async () => {
    ...
    // design (ADR-0051 Asymmetry warrant). Iterates the real catalog so a
    // future credential rephrasing in `~/data/coaches.ts` updates the
    // assertion target automatically, without weakening the guard.
    const doc = await renderBlock();
