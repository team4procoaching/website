...
| `/contact`         | Contact, ContactForm (FormSelect, QuizContextBox, ConfiguratorContextBox, SubscriptionContextBox, ContextBoxShell, ServiceLockedLine)                                     | contact                                                                                               |
...
Conflict resolution priority — strong-intent deep-links win over Quiz: when a
URL carries both a valid Configurator triple and quiz parameters, the
Configurator branch populates its box, preselects the service dropdown, and
short-circuits the quiz branch so quiz hidden fields are never injected on a
Configurator submission.

The visible prefill treatment (locked line + acknowledging headline + context
box) fires on **strong intent only** — a session configurator triple, or a
subscription `?service=<id>` whose `pricingModel === 'subscription'`. A
subscription landing populates `SubscriptionContextBox` (the service name, its
monthly price anchor, and a program-details link suppressed when the service's
detail page is not built) and flips the heading to the `program` variant. A bare
_session_ `?service=<id>` (e.g. `?service=posing` with no configurator triple)
is **not** strong intent: it preselects the editable dropdown and keeps the
conversational headline, with no locked line and no box — preserving the
ServiceCard → Contact flow and the "Not sure yet" escape unchanged.

On a strong-intent landing the contact page makes the service selection visually
read-only: the editable `FormSelect` dropdown is swapped for
`ServiceLockedLine`, a static line that displays the locked service name. The
swap happens in `contactFormController.ts` by toggling the `hidden` class on the
two wrappers (`data-service-select-wrapper` ↔ `data-service-locked-wrapper`).
The page heading itself swaps between three variants — a conversational default
visible on cold loads and quiz arrivals, a transactional variant on configurator
landings, and a program variant on subscription landings — implemented as three
sibling `<span>` elements inside the heading, gated by
`data-contact-headline-mode`. The two context boxes share their card chrome (the
...
A separate `.github/workflows/playwright-a11y.yml` workflow owns the
**page-level accessibility** surface:
[ADR-0057](adr/0057-page-level-accessibility-testing-with-playwright.md). It
scans the canonical 9-URL set on Mobile and Desktop viewports plus four
interactive states (Modal open, MobileMenu open, focus-trap, focus-return) using
a pinned `axe-core` injected into the Playwright browser realm. Lighthouse's
Accessibility category score remains the interim gate until ADR-0057 promotes to
required after three clean nightly runs; at that point the Accessibility
category is removed from the Lighthouse gate per the procedure in
`docs/MAINTENANCE.md` § Playwright A11y. The repo runs two test-runner configs:
`vitest.config.ts` for Vitest tests under `src/`, and `playwright.config.ts` for
Playwright tests under `tests/a11y/`.
