...
| `typescript:S3776` (1 in CSV, 3 new) | `complexity.noExcessiveCognitiveComplexity` | Rule exists; reports four sites. One (`coachDetailModalController.ts:92` `populateCoach`) is in the CSV. The other three (`accordionController.ts:27` callback, `quizModalController.ts:214` `displayResult`, `quizModalController.ts:292` nav callback) are not. Activating the rule today requires four `biome-ignore` suppressions to keep `pnpm lint` green; three of them are debt this ADR is not willing to ship. | **Defer to refactor.** Activate the rule together with the work that refactors `populateCoach` to drop below the threshold. The refactor task carries the activation; this ADR does not.                                                      |
