...
| pnpm version        | `packageManager` field in `package.json` (e.g., `"packageManager": "pnpm@10.26.1"`) | Corepack (built into Node) resolves the exact pnpm binary on local and Netlify; `pnpm/action-setup` is not used in CI — Corepack is sufficient                                                                                  |
