...
| DEBT-260526-01 | minor    | M      | Test             | Add page-level a11y tests for interactive component states: QuizModal step-by-step navigation, ServicesFilter post-filter state, SessionConfigurator post-configuration state, ContactForm prefill branches, and Accordion expand-state. The two specs that shipped (modal-open, mobile-menu-open) cover the proof-of-concept interactive states; these remaining states are deferred follow-ups from the ADR-0057 introduction.                                                            | no         | no                 | ADR-0057                                            |
| DEBT-260526-02 | major    | M      | Component        | Fix the `Accordion` `<dl>` markup defect. Page-level accessibility scanning (ADR-0057) confirmed that the `definition-list` and `dlitem` axe-core violations on routes bearing the `<Accordion>` component are a real markup defect, not a JSDOM rendering artifact. The component-layer test at `src/components/ui/accordion.test.ts` currently suppresses these rules via `disableRules`; that suppression's justification must be re-evaluated and removed once the markup is corrected. | no         | yes                | ADR-0057                                            |
...
| DEBT-260514-01 | 2026-05-26 | —      | ContactForm form-init script extracted to `src/scripts/contactFormController.ts` under the `feat/contact-form-prefill-consistency` stream; ADR-0062 formalised the visibility pattern the controller branches operate against. |
